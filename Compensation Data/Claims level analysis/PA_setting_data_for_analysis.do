...
cd "$log_dir"
log using "PA_setting_data_for_analysis_$out_date.smcl", append
...
=========================================================================================
*****************************************************************************************/
use "$analysis\PA Compensation Data\PA_BIFSG_imputed_data_cleanedforanalysis.dta", clear  
...
mi register regular age crime_homicide crime_sex_assault pct_vacant_housing_units pct_renter_occupied_housing_unit pct_unemployed_over_16 pct_below_poverty amount_paid approved days_decision den_fail_info num_service_providers num_providers_per1000 payment_expense_types payment_lost_wages //register regular all variables for analyses that were not imputed 

//checking imported mi data 
mi describe 


//rename variables to shorten name 
...
//this provides summary statistics for each imputed dataset. It takes a really long time to run so only do it if needed.
*mi xeq: summarize approved ib6.race_w_imp age male crime_homicide crime_sex_assault pct_vac_hous pct_rent_occ pct_unemployed pct_below_poverty num_service_providers

//pooled summary statistics 
sum approved ib6.race_w_imp age male crime_homicide crime_sex_assault pct_vac_hous pct_rent_occ pct_unemployed pct_below_poverty num_service_providers


//create variable for stolen benefit cash payments only 
gen stolen_benefit_cash=0 
replace stolen_benefit_cash=1 if payment_expense_types=="Stolen Benefit Cash"
label variable stolen_benefit_cash "Stolen benefit cash only"

//create indicator for large cities 
gen large_city=0 
replace large_city=1 if inlist(major_city, "Philadelphia", "Scranton", "Allentown", "Pittsburgh", "Reading")

//label variables 
label variable age "Age"
label variable male "Male"
label variable crime_homicide "Homicide Victimization"
label variable crime_sex_assault "Sexual Assault Victimization"
...
label variable payment_lost_wages "Payments for Lost Wages"
...
//summary statistics 
//don't use first dataset with no imputation 
sum approved ib6.race_w_imp age male crime_homicide crime_sex_assault crime_dom_assault pct_below_poverty mi_near_prov den_fail_info payment_lost_wages if imp_num>0

//exporting summary statistics
/*
preserve 
drop if imp_num==0 
outreg2 using "$analysis\PA Compensation Data\claims_descriptives_PA_$out_date.xls", replace sum(log) keep(approved ib6.race_w_imp age male crime_homicide crime_sex_assault crime_dom_assault pct_below_poverty mi_near_prov den_fail_info payment_lost_wages) dec(2) label
restore 
*/
...
//we need to save this file before running the regressions 
save "$analysis\PA Compensation Data\PA_BIFSG_imputed_data_foranalysis_updated.dta", replace  
...
//qui outreg2 using "$analysis\PA Compensation Data\race_distribution_afterimputation_PA.doc", sum (log) keep(ameind api black hispanic other white) replace 
...
//qui outreg2 using "$analysis\PA Compensation Data\race_distribution_raw_PA.doc", sum (log) keep(ameind-other) replace 
*/
