...
corr total_population percent_foreign_born percent_black providers_per_crime per_pop_urban_interp poverty_rate percent_nonwhite
* No correlation greater than .5

//checking correlations with percent foreign born and different races 
corr percent_foreign_born percent_asian percent_hispanic percent_white percent_black percent_nonwhite
*% white and % hispanic .6 

//summary statistics
sum app_crime_rate total_population percent_foreign_born percent_black percent_nonwhite providers_per_crime per_pop_urban_interp poverty_rate

//outreg summary statistics 
outreg2 using "~\The Council of State Governments\JC Research - Documents\RES_NIJVictimComp\data\Analysis\Phase 2 State\NM Compensation Data\cnty_descriptives_NM.xls", replace sum(log) keep(app_crime_rate total_population percent_foreign_born percent_black percent_nonwhite providers_per_crime per_pop_urban_interp poverty_rate) dec(2) label
...
//stata is creating these for observations with missing values as well but they will be dropped from the analysis anyway so we don't have to worry about them 
...
esttab pa_county_perblack using "${dir}\data\Deliverables\nm_county_regressions.rtf", ///
...
//without the outliers total population is not significant 
//we will need to decide how to discuss this in the paper 

//I test % hispanic and % BIPOC as well 
...
xtreg app_crime_rate total_population percent_foreign_born percent_hispanic providers_per_crime per_pop_urban_interp poverty_rate if outlier_hisp==0, vce(cluster county) //total population and % urban are no longer significant 
...
xtreg app_crime_rate total_population percent_foreign_born percent_nonwhite providers_per_crime per_pop_urban_interp poverty_rate if outlier_bipoc==0, vce(cluster county) //total population and % urban are no longer significant 


//since colinearity between the races isn't as high in NM as PA, I test them all together 
reg app_crime_rate total_population percent_foreign_born percent_black percent_hispanic percent_nonwhite providers_per_crime per_pop_urban_interp poverty_rate, vce(cluster county) 
vif

xtreg app_crime_rate total_population percent_foreign_born percent_black percent_hispanic percent_nonwhite providers_per_crime per_pop_urban_interp poverty_rate, vce(cluster county) 
estimates store pa_county_perallrace
//the results are consistent across all the models 


//export tables to word 
esttab pa_county_perhispanic pa_county_perbipoc pa_county_perallrace using "${dir}\data\Deliverables\nm_county_regressions.rtf", ///
    b(%9.3f) se(%9.3f) star(* 0.05 ** 0.01 *** 0.001) ///
    stats(N p, fmt(0 3) labels("Observations"  "p({\i F})")) ///
    title("Longitudinal Linear Random Effects Models Predicting County Application Rates") ///
    mtitle("Percent Hispanic" "Percent BIPOC" "All races") ///
    nonumbers ///
    /*addnotes("Note: )*/ ///
	label append 
